...
myArgs.add_argument("-delobj","--delete_obj",help="delet the object", action="store_true")
myArgs.add_argument("-oname","--object_name",help="enter the object name")
...
def delete_Obj(bname,objname):
    
    objs = s3_client.Object(bname,objname).delete()
    print(f"{objname} from {bname} is deleted successfully!!!!")
...
def mySwitchCase(option):
    ...
    match option:
        case args.delete_obj:
            try:
                if args.bname != empty & args.object_name != empty:
                  delete_Obj(bname=args.bname, objname=args.object_name)
            except:
                print("some error !!!!!!")    
        case args.delete:
